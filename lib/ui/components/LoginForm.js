...
import { KEY_PREFIX } from '../../login_session';
...
class LoginForm extends Tracker.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      message: null,
      waiting: true,
      formState: props.formState || Accounts.user() ? STATES.PROFILE : STATES.SIGN_IN,
  ...
  componentDidMount() {
    ...
    const changeState = Session.get(KEY_PREFIX + 'state');
  ...
  fields() {
    ...
    if (hasPasswordService() && formState === STATES.SIGN_UP) {
      ...
      if (_.includes(['USERNAME_AND_OPTIONAL_EMAIL'], passwordSignupFields())) {
  ...
  buttons() {
    ...
    if (user
      && !_.includes(['EMAIL_ONLY_NO_PASSWORD', 'USERNAME_AND_EMAIL_NO_PASSWORD'], passwordSignupFields())
      && formState === STATES.PROFILE
    ...
    return _.keyBy(loginButtons, 'id');
  }
  ...
  signIn() {
    ...
    if (usernameOrEmail !== null) {
      ...
      if (_.includes(['USERNAME_AND_EMAIL_NO_PASSWORD'], passwordSignupFields())) {
        return this.loginWithoutPassword();
      }
      ...
      if (_.includes(['EMAIL_ONLY_NO_PASSWORD'], passwordSignupFields())) {
  ...
  oauthButtons() {
    ...
    return _.keyBy(oauthButtons, 'id');
  }

  oauthSignIn(service) {
    function capitalService(s) {
    ...
    const loginWithService = Meteor['loginWith' + capitalService(serviceName)];
    ...
    loginWithService(options, (error) => {
      if (error) {
        ...
        loginResultCallback(() => Meteor.setTimeout(() => this.state.onSignedInHook(), 100));
      }
    });
  ...
  signUp(options = {}) {
    ...
    const promise = this.state.onPreSignUpHook(options);
  ...
  showMessage(msg, type, clearTimeout) {
    message = msg.trim();

    if (message) {
      this.setState({ message: { message, type } });
